...
async def async_salveaza_licenta_globala(
    ...
    payload: dict[str, Any] = {
        ...
        CONF_UTILIZATOR: str(utilizator).strip(),
    }
...
async def async_obtine_context_licenta(
    ...
    if intrare is not None:
        entry_utilizator = str(intrare.options.get(CONF_UTILIZATOR, intrare.data.get(CONF_UTILIZATOR, ""))).strip()
        entry_cheie = str(intrare.options.get(CONF_CHEIE_LICENTA, intrare.data.get(CONF_CHEIE_LICENTA, ""))).strip()

    utilizator_final = str(utilizator).strip() if utilizator is not None else (entry_utilizator or storage_utilizator)
...
def _date_licenta_din_storage_sunt_pentru_contextul_curent(
    ...
    return str(date_licenta_globala.get(CONF_CHEIE_LICENTA, "")).strip() == str(cheie_licenta).strip()
    ...
    try:
        async with sesiune.post(URL_API_LICENTA, json=payload, timeout=20) as raspuns:
            ...
            return RezultatLicenta(
                valida=bool(data.get("valid", False)) and data.get("status") in STATUSURI_ACCEPTATE,
                status=data.get("status", LICENTA_STATUS_NECUNOSCUT),
                plan=data.get("plan"),
...
async def async_verifica_licenta(hass: HomeAssistant, intrare: ConfigEntry | None = None) -> RezultatLicenta:
    if intrare is None:
        utilizator, cheie, storage = await async_obtine_context_licenta(hass)
        if not utilizator:
            return RezultatLicenta(True, LICENTA_STATUS_TRIAL, plan="trial_local", verificata_la=_acum_utc_iso())
    else:
...
def valideaza_rezultat_licenta(rezultat: RezultatLicenta) -> None:
    if rezultat.valida and rezultat.status in STATUSURI_ACCEPTATE:
        return
    if rezultat.eroare_conectare:
    ...
    raise EroareLicenta("licenta_necunoscuta")
